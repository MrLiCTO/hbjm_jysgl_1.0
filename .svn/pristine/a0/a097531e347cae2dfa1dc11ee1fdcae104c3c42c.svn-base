...
<script type="text/javascript">
		...
		function couteadelete(){
			_confirm('您确定要删除选择的授课教师吗?',null,
					  function(){
						...
						var indexs = [];//选择记录的序号
						var rows = $('#couteaslist').datagrid('getSelections');
						for(var i=0;i<rows.length;i++){
							var index=$('#couteaslist').datagrid('getRowIndex',rows[i]);
							indexs.push(index);
						}
						if(rows.length>0){
							$("#indexs").val(indexs.join(','));
							//由于删除和保存对同一个form进行操作，所以这里给yycgdmxForm的action重新赋值
							$("#sysuserqueryForm").attr("action",'${baseurl}course/couteas/couteadel_submit.action?cid=${cid}');
	...
	var toolbar_v = [ {//工具栏
		...
		handler : function() {
			...
			createmodalwindow("教师列表", 800, 420, '${baseurl}course/couteas/allteaslist.action?cid=${cid}');
		}
	},{
		id : 'btndel',
		text : '批量刪除',
	...
	$(function() {
		$('#couteaslist').datagrid({
			title : '授课教师列表',//数据列表标题
			nowrap : true,//单元格中的数据不换行，如果为true表示不换行，不换行情况下数据加载性能高，如果为false就是换行，换行数据加载性能不高
			striped : true,//条纹显示效果
			url : '${baseurl}course/couteas/couteaslist_result.action?cid=${cid}',//加载数据的连接，引连接请求过来是json数据
	...
	function queryuser(){
		...
		$('#couteaslist').datagrid('load',formdata);
	}
  ...
  <input type="hidden" id="indexs" name="indexs"/>
	...
	<TABLE border=0 cellSpacing=0 cellPadding=0 width="99%" align=center>
		<TBODY>
			<TR>
				<TD>
					<table id="couteaslist"></table>
				</TD>
			</TR>
		</TBODY>
	</TABLE>
</form>
<form id="sysuserdeleteform" action="${baseurl}course/couteas/delcoutea_submit.action" method="post">
