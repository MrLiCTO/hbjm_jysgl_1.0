...
<script type="text/javascript">
		...
		//采购药品删除
		function cgcoudelete(){
			_confirm('您确定要删除选择的课程组课程吗?',null,
					  function(){
	...
	var columns_v = [ [{
		...
		formatter : function(value, row, index) {//通过此方法格式化显示内容,value表示从json中取出该单元格的值，row表示这一行的数据，是一个对象,index:行的序号
			return "<a href=javascript:deletecgcou('"+row.cid+"')>删除</a>"
		}
	} 
	...
	var toolbar_v = [ {//工具栏
		...
		handler : function() {
			...
			createmodalwindow("课程列表", 600, 300, '${baseurl}coursegroup/cgcous/allcouslist.action?cgid=${cgid}');
		}
	},{
		id : 'btndel',
		text : '批量刪除',
	...
	$(function() {
		$('#cgcouslist').datagrid({
			title : '课程组课程列表',//数据列表标题
			nowrap : true,//单元格中的数据不换行，如果为true表示不换行，不换行情况下数据加载性能高，如果为false就是换行，换行数据加载性能不高
			striped : true,//条纹显示效果
			url : '${baseurl}coursegroup/cgcous/cgcouslist_result.action?cgid=${cgid}',//加载数据的连接，引连接请求过来是json数据
...
<form id="sysuserdeleteform" action="${baseurl}coursegroup/cgcous/delcgcou_submit.action" method="post">
