...
<script type="text/javascript">
		...
		//采购药品删除
		function couteaadd(){
			_confirm('您确定要添加选择的授课教师吗?',null,
					  function(){
								
						var indexs = [];//选择记录的序号
						var rows = $('#allteaslist').datagrid('getSelections');
						for(var i=0;i<rows.length;i++){
							var index=$('#allteaslist').datagrid('getRowIndex',rows[i]);
							indexs.push(index);
						}
						if(rows.length>0){
							$("#indexs").val(indexs.join(','));
							//由于删除和保存对同一个form进行操作，所以这里给yycgdmxForm的action重新赋值
							$("#sysuserqueryForm").attr("action",'${baseurl}course/couteas/couteaadd_submit.action?cid=${cid}');
							jquerySubByFId('sysuserqueryForm', couteaadd_callback, null);
						}else{
							alert_warn("请选择要添加的授课教师");
						}
					  }
					)
		}
	...
	var columns_v = [ [{
		...
		formatter : function(value, row, index) {//通过此方法格式化显示内容,value表示从json中取出该单元格的值，row表示这一行的数据，是一个对象,index:行的序号
			return "<a href=javascript:addcoutea('"+row.teaid+"')>添加</a>"
		}
	} 
	...
	$(function() {
		$('#allteaslist').datagrid({
			title : '授课教师列表',//数据列表标题
			nowrap : true,//单元格中的数据不换行，如果为true表示不换行，不换行情况下数据加载性能高，如果为false就是换行，换行数据加载性能不高
			striped : true,//条纹显示效果
			url : '${baseurl}coursegroup/cgteas/allteaslist_result.action?cgid=${cgid}',//加载数据的连接，引连接请求过来是json数据
	...
	function queryuser(){
		...
		$('#allteaslist').datagrid('load',formdata);
	}
  ...
  <input type="hidden" id="indexs" name="indexs"/>
	...
	<TABLE border=0 cellSpacing=0 cellPadding=0 width="99%" align=center>
		<TBODY>
			<TR>
				<TD>
					<table id="allteaslist"></table>
				</TD>
			</TR>
		</TBODY>
	</TABLE>
</form>
<form id="sysuseraddform" action="${baseurl}coursegroup/cgteas/addcgtea_submit.action" method="post">
  <input type="hidden" id="add_id" name="teaid"/>
