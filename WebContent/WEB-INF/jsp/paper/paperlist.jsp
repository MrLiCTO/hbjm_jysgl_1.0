...
<title>教师论文信息管理</title>

<script type="text/javascript">


		function paperdelete(){
			_confirm('您确定要删除选择的论文信息吗?',null,
					  function(){
								
						var indexs = [];//选择记录的序号
						var rows = $('#paperlist').datagrid('getSelections');
						for(var i=0;i<rows.length;i++){
							var index=$('#paperlist').datagrid('getRowIndex',rows[i]);
							indexs.push(index);
						}
						if(rows.length>0){
							$("#indexs").val(indexs.join(','));
							//由于删除和保存对同一个form进行操作，所以这里给yycgdmxForm的action重新赋值
							$("#paperqueryForm").attr("action",'${baseurl}paper/paperdelete_submit.action');
							jquerySubByFId('paperqueryForm', paperdelete_callback, null);
						}else{
							alert_warn("请选择要删除的论文信息");
						}
					  }
					)
		}
		function paperdelete_callback(data) {
	...
	var columns_v = [ [{
		...
		field : 'paid',//对应json中的key
		hidden : true,//隐藏
		formatter: function(value,row,index){
			return '<input type="hidden" name="paperCustoms['+index+'].paid" value="'+value+'" />';
		}
	},{
		field : 'lwtm',//对应json中的key
		title : '论文题目',
		width : 120
	},{
		field : 'fbqkmc',//对应json中的key
		title : '发表期刊名称',
		width : 120
	}, {
		field : 'fbsjstr',//对应json中的key
		title : '发表时间 ',
		width : 70
	}, {
		field : 'lwdcmc',//对应json中的key
		title : '论文档次',
		width : 70
	}, {
		field : 'jszkmc',//对应json中的key
		title : '检索状况',
		width : 70
	}, {
		field : 'lwlbmc',//对应json中的key
		title : '论文类别',
		width : 70
	}, {
		...
		formatter : function(value, row, index) {//通过此方法格式化显示内容,value表示从json中取出该单元格的值，row表示这一行的数据，是一个对象,index:行的序号
			return "<a href=javascript:deletepaper('"+row.paid+"')>删除</a>&nbsp;&nbsp;&nbsp;<a href=javascript:editpaper('"+row.paid+"')>编辑</a>"
		}
	} 
	...
	var toolbar_v = [ {//工具栏
		...
		handler : function() {
			...
			createmodalwindow("添加论文信息", 900, 400, '${baseurl}paper/addpaper.action');
		}
	},{
		id : 'btndel',
		text : '批量刪除',
	...
	$(function() {
		$('#paperlist').datagrid({
			...
			striped : true,//条纹显示效果
			url : '${baseurl}paper/paperlist_result.action',//加载数据的连接，引连接请求过来是json数据
			idField : 'paid',//此字段很重要，数据结果集的唯一约束(重要)，如果写错影响 获取当前选中行的方法执行
			loadMsg : '',
			columns : columns_v,
	...
	function querypaper(){
		...
		var formdata = $("#paperqueryForm").serializeJson();
		$('#paperlist').datagrid('load',formdata);
	}
	
	//刪除用戶
	function deletepaper(taid){
		...
		_confirm('您确认删除吗？',null,function (){
			...
			jquerySubByFId('paperdeleteform',deletepaper_callback,null,"json");
	...
	//删除用户的回调
	function deletepaper_callback(data){
		message_alert(data);
		//刷新 页面
		//在提交成功后重新加载 datagrid
		//取出提交结果
	...
	//修改用戶
	function editpaper(paid){
		//打开一个窗口，用户添加页面
		//参数：窗口的title、宽、高、url地址
		createmodalwindow("修改论文信息", 900, 400, '${baseurl}paper/editpaper.action?paid='+paid);
	}
</script>
				...
				<TBODY>
					<TR>
						...
						<INPUT id="paperCustom.fbsj_start"
								name="paperCustom.fbsj_start"   value="<fmt:formatDate value="${paperCustom.lxsj_start}" pattern="yyyy-MM-dd"/>"
								onfocus="WdatePicker({isShowWeek:false,skin:'whyGreen',dateFmt:'yyyy-MM-dd'})"
								style="width: 80px" />
						---
						<INPUT id="paperCustom.fbsj_end"
								name="paperCustom.fbsj_end"   value="<fmt:formatDate value="${paperCustom.lxsj_end}" pattern="yyyy-MM-dd"/>"
								onfocus="WdatePicker({isShowWeek:false,skin:'whyGreen',dateFmt:'yyyy-MM-dd'})"
								style="width: 80px" />	
					...
					<TR>
						...
						<a id="btn" href="#" onclick="querypaper()" class="easyui-linkbutton" iconCls='icon-search'>查询</a>
				  		</td>
...
<form id="paperdeleteform" action="${baseurl}paper/deletepaper_submit.action" method="post">
  <input type="hidden" id="delete_id" name="paid"/>
</form>
